...
def train(model, opt, lr_scheduler, train_loader, test_loader,
    ...
    for epoch in range(math.ceil(args.num_epochs)):
        ...
        if lr_scheduler is not None:
            ...
            # For fedavg: use the constant LR from args.lr if set,
            # otherwise fall back to lr_scale
...
if __name__ == "__main__":
    ...
    # args.dataset_name = '~/datasets/cifar10/'
    ...
    # print("Grad vec min/max:", grad.min().item(), grad.max().item())
    # print("Grad vec l2 norm:", torch.norm(grad).item())
    # print("Grad vec mean/std:", grad.mean().item(), grad.std().item())
    # print("Grad vec sparsity:", (grad==0).float().mean().item())
    # print("Grad vec nan count:", torch.isnan(grad).sum().item())
    # print("Grad vec inf count:", torch.isinf(grad).sum().item())
    # print("Grad vec size:", grad.numel())


    # print("Total params:", sum(p.numel() for p in model.parameters() if p.requires_grad))
    # print("Grad vec device:", grad.device)
    ...
    if args.mode != "fedavg":
        ...
        lr_scheduler = None  # FedAvg uses constant lr
    ...
    print("Grad vec mean/std:", grad.mean().item(), grad.std().item())
    print("Grad vec nan count:", torch.isnan(grad).sum().item())
    print("Grad vec zero count:", (grad == 0).sum().item())
    print("Grad vec positive count:", (grad > 0).sum().item())
    print("Grad vec negative count:", (grad < 0).sum().item())
    print("Grad vec size:", grad.numel())
    ...
    print(grad)
    print("----")
    print("1st 10 elements of grad vec:", grad[:10])
