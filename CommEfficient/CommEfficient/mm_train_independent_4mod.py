"""
4-modality Independent Compression training: :class:`FedMultiModal4` +
:class:`IndependentCompression4`.

Summation fusion of all four modalities, then original FetchSGD Count
Sketch of the full gradient (Rothchild et al.) — the 4-modality analogue of
...
Use ``--skip_map`` (retrieval MAP is only defined for the 2-modality
``extract_fused`` API in ``mm_train.py``).
"""
...
import models
from models.independent_compression_net_4 import IndependentCompression4

# So ``parse_args --model IndependentCompression4`` is a valid choice
# without editing models/__init__.py (mirrors mm_train_independent.py).
models.IndependentCompression4 = IndependentCompression4
...
def _load_prepare_stats(args):
    ...
    if not os.path.isfile(path):
        raise FileNotFoundError(
            ...
            f"prepare_mhealth_mm.py (or the relevant 4-modality prepare "
            f"script) first."
        )
...
if __name__ == "__main__":
    ...
    if not getattr(args, "skip_map", False):
        raise ValueError(
            "mm_train_independent_4mod requires --skip_map "
    ...
    print(f"IndependentCompression4 config: {model_config}")
    print(
        "Fusion: summation (sum of 4 refined modality features).  "
        "Gradient compression: original FetchSGD Count Sketch."
    )
    model = IndependentCompression4(**model_config)
