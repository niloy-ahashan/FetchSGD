...
from mm_sketch_separated import get_index_maps, server_step_separated
import torch
...
class FedModel:
    def __init__(self, input_model, compute_loss, args,
        ...
        if args.local_momentum > 0:
            ...
            # # New: allocate only for active workers
            # shape = (args.num_workers, args.grad_size)
            # g_client_velocities = torch.zeros(shape, device=args.device).share_memory_()
            # g_client_errors = torch.zeros(shape, device=args.device).share_memory_()
        ...
        # if args.mode == "sketch" and len(self.update_forward_grad_ps) == 0:
        #     # Start at least one server helper for sketch mode
        #     self.update_forward_grad_ps = [ _server_helper_sketched ]
        ...
        # torch.distributed.init_process_group(backend="gloo" if args.device == "cpu" else "nccl", rank=0,
        #                                      world_size=world_size)
        ...
        torch.distributed.init_process_group(
            backend=backend,
            init_method="tcp://127.0.0.1:29500",  # ✅ explicit rendezvous
            rank=0,
            world_size=world_size
        )
        ...
        # always allocate minibatch gradient as dense vector
        # g_minibatch_gradient = torch.zeros(args.grad_size).to(args.device)
...
class FedOptimizer(torch.optim.Optimizer):
    ...
    def get_lr(self):
        ...
        if self.args.mode == "fedavg":
            # Force FedAvg to use a single scalar LR
...
def args2sketch(args):
    return CSVecFed(d=args.grad_size, c=args.num_cols,
                 r=args.num_rows, device=args.device,
                 numBlocks=args.num_blocks)

    #for MN:
    # return MN(d=args.grad_size, c=args.num_cols,
    #           r=args.num_rows, device=args.device,
    #           numBlocks=args.num_blocks,
    #           use_mn=args.use_mn,  # Pass the use_mn flag
    #           m=args.mn_num_fake_items)  # Pass the number of fake items


def get_server_update(gradient, Vvelocity, Verror, args, lr):
    if (args.mode == "sketch"
            and getattr(args, "mm_sketch_separated", False)):
        index_maps = get_index_maps(args, args.device)
        return server_step_separated(
            gradient, Vvelocity, Verror, args, lr, index_maps,
        )
...
def _server_helper_sketched(sketched_grad, Vvelocity, Verror, args, lr):
    ...
    # print("Sketch table before accumulateVec:")
    # print(sketch.table.view(-1)[:10])
    sketch.accumulateTable(Verror)

    # # NEW: Measure noise if MN is enabled
    # if args.use_mn:
    #     measured_noise = sketch.measure_noise()
    #     # print(f"Server measured noise: {measured_noise:.4f}")
    ...
    # print("1st 10 elements of update vec:", update[:10])
    ...
    # print("1st 10 elements of sketched_update vec:", sketched_update.view(-1)[:10])
    ...
    if args.error_type == "virtual":
        ...
        # print("1st 10 elements of Verror vec before zeroing:", Verror.view(-1)[:10])
        Verror[nz[:,0],nz[:,1]] = 0
        # print("1st 10 elements of Verror vec:", Verror.view(-1)[:10])
    ...
    # print("1st 10 elements of Vvelocity vec:", Vvelocity.view(-1)[:10])
