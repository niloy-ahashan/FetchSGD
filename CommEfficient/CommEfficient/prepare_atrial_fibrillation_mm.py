#!/usr/bin/env python3
"""
Atrial Fibrillation (UEA / TS format) → FedMultiModal ``data.npz``.

**2** ECG-derived series × **640** time steps; labels **n**, **s**, **t** (3 classes).

Two modalities (one series each), same pattern as other TS splits:
  • img_* — dimension 0  (length 640)
  • txt_* — dimension 1  (length 640)

Labels stored as integers 0..2 for CrossEntropy (``num_classes=3``).

Usage
-----
  python prepare_atrial_fibrillation_mm.py \\
...
_NUM_DIMS = 2
_SERIES_LEN = 640
_LABEL_TO_ID = {"n": 0, "s": 1, "t": 2}


def _parse_ts_file(path: str) -> tuple[np.ndarray, np.ndarray]:
    """Return X (N, 2, T), y (N,) with labels 0..2."""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing AtrialFibrillation_TRAIN.ts / _TEST.ts",
    )
    p.add_argument("--out_dir", type=str, required=True)
    args = p.parse_args()

    ts_dir = os.path.abspath(args.ts_dir)
    train_path = os.path.join(ts_dir, "AtrialFibrillation_TRAIN.ts")
    test_path = os.path.join(ts_dir, "AtrialFibrillation_TEST.ts")
    ...
    # dim 0 → img, dim 1 → txt
    img_tr = x_tr[:, 0, :].reshape(x_tr.shape[0], -1).astype(np.float32)
    txt_tr = x_tr[:, 1, :].reshape(x_tr.shape[0], -1).astype(np.float32)
    img_te = x_te[:, 0, :].reshape(x_te.shape[0], -1).astype(np.float32)
    txt_te = x_te[:, 1, :].reshape(x_te.shape[0], -1).astype(np.float32)
    ...
    print(
        f"  num_classes=3 (n/s/t); img_dim={img_dim}, txt_dim={txt_dim}"
    )
