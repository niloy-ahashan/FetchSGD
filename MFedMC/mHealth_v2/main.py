...
def _default_dataset_dir():
    ...
    return os.path.join(repo, "datasets", "mhealth_mm")
...
def main():
    ...
    print("Loading mHealth Acc/Gyro/Mag/ECG features...")
    client_data, global_test, meta = load_mhealth_dir(
        dataset_dir,
    ...
    arg_dims = [args.acc_dim, args.gyro_dim, args.mag_dim, args.ecg_dim]
    if meta["mod_dims"] != arg_dims:
        print(
            f"Warning: data mod_dims={dict(zip(MODALITIES, meta['mod_dims']))} "
            f"but args={dict(zip(MODALITIES, arg_dims))}. Using data dims."
        )
        args.acc_dim, args.gyro_dim, args.mag_dim, args.ecg_dim = meta["mod_dims"]
    ...
    if args.eval_on_global_test:
        print("Evaluating fusion on the official mHealth test set.")
    ...
    k = args.num_classes
    mod_dims = [args.acc_dim, args.gyro_dim, args.mag_dim, args.ecg_dim]
    global_modality_encoders = [
        VectorMLP(d, k, args.feat_dim, args.mm_dropout, use_refiner=args.use_refiner)
        for d in mod_dims
    ]
    ...
    print(f"VectorMLP v2 config: feat_dim={args.feat_dim} "
          f"use_refiner={args.use_refiner} dropout={args.mm_dropout}")
    for name, m in zip(MODALITIES, global_modality_encoders):
        n_params = sum(p.numel() for p in m.parameters() if p.requires_grad)
        print(f"{name} model params: {n_params}")

    print("Starting MFedMC federated learning on Acc + Gyro + Mag + ECG...")
    (
        acc,
        mod_counts,
        upload_bytes_round,
    ...
    ref_str = "ref" if args.use_refiner else "noref"
    file_name = os.path.join(
        results_dir,
        f"MFedMC_mHealth_v2_mm_{args.partition}_feat{args.feat_dim}_{ref_str}_"
        f"Top_{args.top_shap}_ShapCommRec_{mw_str}_"
    ...
    np.savez(
        ...
        feat_dim=np.array(args.feat_dim),
        use_refiner=np.array(args.use_refiner),
    )
